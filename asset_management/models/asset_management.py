...
class Asset(models.Model):
    ...
    equipment_id = fields.Many2one('maintenance.equipment', string="Associated Product", help="Select the product used in this asset from available options")
...
class AssetTransferEntry(models.Model):
    ...
    @api.onchange('transfer_employee_id', 'assign_date', 'return_date', 'status')
    def _onchange_return_asset_allotment(self):
        for rec in self:
            if rec.asset_id and rec.asset_id.equipment_id:
                equipment = rec.asset_id.equipment_id
                if equipment:
                    if rec.status == 'returned' and rec.return_date:
                        # Ensure equipment exists before updating
                        equipment.employee_id = False
                    elif rec.status == 'assigned' and rec.transfer_employee_id and rec.assign_date:
                        # Ensure equipment exists before updating
                        equipment.employee_id = rec.transfer_employee_id.id
