...
class MonthlyProgressReport(models.TransientModel):
    ...
    def _generate_excel_report(self, data):
        ...
        for user, weeks in data.items():
            ...
            row_data = [user.id, user.name, user.employee_id.department_id.name or '', user.employee_id.ticket_resolved, user.employee_id.avg_resolution_time, user.employee_id.CAST, user.employee_id.billable_hours]
