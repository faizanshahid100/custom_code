...
class DailyProgressReport(models.TransientModel):
    ...
    def action_generate_report(self):
        ...
        if self.department_id and not self.user_ids:
            employee_ids = self.env['res.users'].search([('department_id', '=', self.department_id.id)])
        else:
            employee_ids = self.user_ids

        # Fetch records and all users
        records, all_users = self._get_records(employee_ids, self.date_from, self.date_to)
    ...
    def _generate_excel_report(self, data):
        ...
        for user, weeks in data.items():
            row_data = [user.id, user.name, user.employee_id.department_id.name or '', user.employee_id.d_ticket_resolved, user.employee_id.d_avg_resolution_time, user.employee_id.d_CAST, user.employee_id.d_billable_hours]
