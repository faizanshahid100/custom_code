...
class AttendanceDevice(models.Model):
    ...
    firmware_version = fields.Char(string='Firmware Version', readonly=True,
                                   help="The firmware version of the device which will be filled automatically when you hit the 'Get Device Info' button.", tracking=True)
    serialnumber = fields.Char(string='Serial Number', readonly=True,
                               help="The serial number of the device which will be filled automatically when you hit the 'Get Device Info' button.", tracking=True)
    oem_vendor = fields.Char(string='OEM Vendor', readonly=True,
                             help="The OEM Vendor of the device which will be filled automatically when you hit the 'Get Device Info' button.", tracking=True)
    platform = fields.Char(string='Platform', readonly=True,
                           help="The Platform of the device which will be filled automatically when you hit the 'Get Device Info' button.", tracking=True)
    fingerprint_algorithm = fields.Char(string='Fingerprint Algorithm', readonly=True,
                                        help="The Fingerprint Algorithm (aka ZKFPVersion) of the device which will be filled automatically when you hit the 'Get Device Info' button.", tracking=True)
    device_name = fields.Char(string='Device Name', readonly=True,
                              help="The model of the device which will be filled automatically when you hit the 'Get Device Info' button.", tracking=True)

    work_code = fields.Char(string='Work Code', readonly=True,
                            help="The Work Code of the device which will be filled automatically when you hit the 'Get Device Info' button.", tracking=True)
    ...
    device_user_ids = fields.One2many('attendance.device.user', 'device_id', string='Device Users',
                                      help='List of Users stored in the attendance device', tracking=True)
    ...
    mapped_employee_ids = fields.Many2many('hr.employee', 'mapped_device_employee_rel', string='Mapped Employees',
                                           ...
                                           help="List of employees that have been mapped with this device's users", tracking=True)
    ...
    umapped_device_user_ids = fields.One2many('attendance.device.user', 'device_id', string='Device Users',
                                              ...
                                              help='List of Device Users that have not been mapped with an employee', tracking=True)
    ...
    unmapped_employee_ids = fields.Many2many('hr.employee', 'device_employee_rel', 'device_id', 'employee_id',
                                             ...
                                             help="The employees that have not been mapped with any user of this device", tracking=True)
    ...
    auto_clear_attendance = fields.Boolean(string='Auto Clear Attendance Data', default=False, tracking=True,help='Check this to clear all device attendance data after download into Odoo')
    ...
    unique_uid = fields.Boolean(string='Unique UID', default=True, required=True, tracking=True,help='Some Bad Devices allow uid duplication. In this case, uncheck this field. But it is recommended to change your device.')
    last_attendance_download = fields.Datetime(string='Last Sync.', readonly=True,
                                               help='The last time that the attendance data was downlowed from the device into Odoo.', tracking=True)

    map_before_dl = fields.Boolean(string='Map Student Before Download', default=True,
                                   help='Always try to map users and employees (if any new found) before downloading attendance data.', tracking=True)
    create_employee_during_mapping = fields.Boolean(string='Generate Student During Mapping', default=False,
                                                    help="If checked, during mapping between Device's Users and company's employees, umapped device"
                                                         " users will try to create a new employee then map accordingly.", tracking=True)

    download_error_notification = fields.Boolean(string='Download Error Notification', default=True,help='Enable this to get notified when data download error occurs.')
    debug_message = fields.Boolean(string='Debug Message', default=False,
                                   help="If checked, debugging messages will be posted in"
                                        " OpenChatter for debugging purpose", tracking=True)

    user_attendance_ids = fields.One2many('user.attendance', 'device_id', string='Attendance Data', readonly=True, tracking=True)
    total_att_records = fields.Integer(string='Attendance Records', compute='_compute_total_attendance_records', tracking=True)
    finger_template_ids = fields.One2many('finger.template', 'device_id', string='Finger Template', readonly=True, tracking=True)
    ...
    password = fields.Char(string='Password',help="The password to authenticate the device, if required")
    ...
    #                                 #     if count_18 < 13.0:
    #                                 #         users[0].unlink()
    #                                 elif count_18 > 0 and str_utc_timestamp_add.date()== users[1].date == users[0].date:
    #                                     users[0].valid = False
    #                                     break
        ...
        for r in self:
            ...
            for attendance in attendance_data:
                ...
                if user_id:
                    ...
                    # if not duplicate_attend and str_utc_timestamp > time_date:
                    print(duplicate_attend)
                    print("duplicate_attend duplicate_attend duplicate_attend ")
                    if not duplicate_attend:
                        print("if duplicate_attend if duplicate_attend ")
                        # try:
                        vals = {
                          'valid': True,
                          'device_id': r.id,
                          ...
                          'attendance_state_id': attendance_states.get(attendance.punch, None)

                          # 'attendance_state_id': 1
                        }
                        print(attendance_states)
                        # print(attendance_states.attendance)
                        print("attendance_states attendance_states attendance_states attendance_states ")
                        print(vals)
                        ...
                        if users:
                            ...
                            for user in users:
                                ...
                                if len(users) >= 2 and len(users) % 2 == 0:
                                    ...
                                    timestamp = str_utc_timestamp_add - users[1].timestamp
                                    count_18 = timestamp.total_seconds() / 3600
                                    if users[0].date == date_condition and count_18 > 24:
                                        reduce_date = 8
                                    if count_18 < 0:
                                        reduce_date = 8

                                    elif cont < user_len and count_18 < 24.0 and count_18 > 0:
                                        # user.unlink()
                                        break
                                    else:
                                        break
