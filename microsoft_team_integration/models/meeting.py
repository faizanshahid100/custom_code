                    ...
                    body = {
                        "message": {
                            ...
                            "body": {
                                "contentType": "HTML",
                                "content": (
                                    f"<div><a>You Are Invited to Meeting</a>"
                                    f"<p>{self.name}</p></div>"
                                    f"<div><ul>"
                                    f"<li>Meeting URL: <a target='_blank' href='{str(self.team_url)}'>Join a Team Meeting</a></li>"
                                    f"<li>Meeting Date: <a>{fields.Datetime.context_timestamp(self, self.start).isoformat(' ')}</a></li>"
                                    f"<li>Meeting Description: <a>{self.description}</a></li>"
                                    f"</ul></div>"
                                )
                            },
                            "toRecipients": [
                                {"emailAddress": {"address": i.email}}
                            ],
                            "ccRecipients": [
    ...
    def post_request_team_meet(self):
        ...
        if client_details.access_token and client_details.refresh_token:
            ...
            if response.status_code == 201:
                ...
                raise AccessError("Please Authenticate with Microsoft Team.")
